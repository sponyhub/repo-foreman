...
const RUNS_DIR_NAME = '.repo-foreman/runs'
const WORKTREES_DIR_NAME = '.repo-foreman/worktrees'
...
export async function resumeCommand(options) {
  ...
  const resolvedModel = options.model ?? manifest?.runtime_config?.model ?? manifest?.model ?? DEFAULT_MODEL
  const resolvedReasoningEffort =
    options.effort ??
    manifest?.runtime_config?.reasoning_effort ??
    manifest?.model_reasoning_effort ??
    DEFAULT_MODEL_REASONING_EFFORT
  manifest.model = resolvedModel
  manifest.model_reasoning_effort = resolvedReasoningEffort
...
export async function doctorCommand(options) {
  ...
  try {
    ...
    checks.push({
      ...
      detail: support.supported ? 'required Codex options are available' : `missing: ${support.missingFlags.join(', ')}`,
    })
  } catch (error) {
  ...
  console.log('\nRepoForeman defaults to workspace-write, web search off, and the strict policy preset.')
...
function buildRuntimeConfigSnapshot(options) {
  return {
    ...
    reasoning_effort: options?.effort ?? DEFAULT_MODEL_REASONING_EFFORT,
...
async function preflight(mainRepoRoot, runId, options, runDir, branchName, ui) {
  ...
  if (!useWorktree) {
    const result = await ensureCleanWorkingTree(mainRepoRoot, {
      ...
      stashMessage: `repo-foreman ${runId}`,
    })
    stashed = result.stashed
  }

  const gitAuthorName = options.git_author_name ?? 'RepoForeman'
  const gitAuthorEmail = options.git_author_email ?? 'repo-foreman@local'
...
async function resolvePolicy(options, runDir) {
  const policy = await loadPolicy({
    ...
    onWarning: (message) => {
      process.stderr.write(`[repo-foreman:policy] WARN: ${message}\n`)
    },
  })
...
function createUi({ runId, verbose, prettyEvents }) {
  ...
  const prefix = `[repo-foreman:${runId}]`
...
async function preflightPrereqs(codexBin, repoRoot) {
  ...
  try {
    ...
    if (!contract.supported) {
      throw new Error(`missing required Codex options: ${contract.missingFlags.join(', ')}`)
    }
  } catch (error) {
    throw new Error(`codex CLI is incompatible with this RepoForeman release (${error?.message ?? error})`)
  }
