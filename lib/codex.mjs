...
const REQUIRED_ROOT_FLAGS = Object.freeze([
  '--ask-for-approval',
  '--search',
])
...
export function assessCodexCliContract({ cliVersion = null, rootHelpText = '', execHelpText = '' } = {}) {
  const rootOptionNames = parseHelpOptionNames(rootHelpText)
  const execOptionNames = parseHelpOptionNames(execHelpText)
  const missingFlags = [
    ...REQUIRED_ROOT_FLAGS.filter((flag) => !rootOptionNames.has(flag)),
    ...REQUIRED_EXEC_FLAGS.filter((flag) => !execOptionNames.has(flag)),
  ]
  return {
...
export async function detectCodexCliContract({ codexBin, repoRoot }) {
  ...
  const [cliVersion, rootHelpText, execHelpText] = await Promise.all([
    readCodexCliText(resolvedCodexBin, ['--version'], { cwd: repoRoot }),
    readCodexCliText(resolvedCodexBin, ['--help'], { cwd: repoRoot }),
    readCodexCliText(resolvedCodexBin, ['exec', '--help'], { cwd: repoRoot }),
  ])
  return assessCodexCliContract({ cliVersion, rootHelpText, execHelpText })
}
