...
export async function runSelfTest({ repoRoot = process.cwd(), cleanup = true } = {}) {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'repo-foreman-self-test-'))
...
async function createFixtureRepo(repoRoot) {
  ...
  await writeFile(
    ...
    `${JSON.stringify({ name: 'repo-foreman-self-test-fixture', private: true, scripts: { test: NOOP_COMMAND } }, null, 2)}\n`,
    'utf8',
  )
  await writeFile(path.join(repoRoot, 'README.md'), '# RepoForeman self-test fixture\n', 'utf8')
  await writeFile(path.join(repoRoot, '.gitignore'), '.repo-foreman/\n', 'utf8')
  await execFileAsync('git', ['init', '--quiet', '-b', 'main'], { cwd: repoRoot })
  await execFileAsync('git', ['config', 'user.name', 'RepoForeman Self-Test'], { cwd: repoRoot })
  await execFileAsync('git', ['config', 'user.email', 'self-test@repo-foreman.local'], { cwd: repoRoot })
...
async function findRunDirByRunId(repoRoot, runId) {
  const runsRoot = path.join(repoRoot, '.repo-foreman', 'runs')
...
if (args.length === 1 && args[0] === '--help') {
  console.log('--ask-for-approval --search')
  process.exit(0)
}

if (args[0] === 'exec' && args[1] === '--help') {
  console.log('--cd --json --output-last-message --output-schema --sandbox')
  process.exit(0)
}
...
for (const rootFlag of ['--ask-for-approval', '--search']) {
  const flagIndex = args.indexOf(rootFlag)
  if (flagIndex > execIndex) {
    console.error('Root option passed after exec: ' + rootFlag)
    process.exit(3)
  }
}

if (!args.includes('approval_policy="on-request"') || !args.includes('web_search="disabled"')) {
  console.error('Missing deterministic Codex config overrides')
  process.exit(4)
}
